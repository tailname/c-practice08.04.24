...
void Deanary::addMarksToAll(const Group& group) {
    for (int i = 0; i < group.getStudents().size(); ++i) {
        ...
        unsigned int mark;
        rand_s(&mark);
        group.getStudents()[i]->addmark(mark % 10);
    }
}
...
void Deanary::getStatistics() {
    ...
    for (Group* group : groups) {
        ...
        if (group->GetheadID() !=-1)
            std::wcout <<
                         L"The head of group: " <<
                         group->GetheadID() << std::endl;
        std::wcout << L"The average mark in the group is: " <<
                      group->getAveragemark() <<
                      std::endl;
        std::wcout << L"Students of group \" " <<
                      group->GetGroupTitle() << L" \": " <<
                      std::endl;
        for (Student* st : group->getStudents()) {
            std::wcout << "ID: " << st->GetID() <<
                          " Name: " << st->GetFIO() <<
                          " Marks: ";
            for (int mark : st->GetMarks()) {
                std::wcout << mark << " ";
            }
            std::wcout << " Average mark of student: " <<
...
void Deanary::moveStudents(const std::vector<Student*>& students,
                           Group* group) {
...
void Deanary::saveStaff() {
    ...
    for (int i = 0; i < groups.size(); ++i) {
        for (int j = 0; j < groups[i]->getStudents().size(); ++j) {
            out << groups[i]->getStudents()[j]->GetID()<< L" " <<
                   groups[i]->getStudents()[j]->GetFIO() << L" " <<
                   groups[i]->GetGroupTitle();
            for (int k = 0;
                k < groups[i]->getStudents()[j]->GetMarks().size();
                ++k) {
...
void Deanary::fireStudents(const std::vector<Student*>& students) {
    for (Student* student : students) {
        ...
        student->GetGroup()->removeStudent(student->GetID());
        if (gr != nullptr &&
            student->GetID() == gr->GetheadID()) gr->chooseHead();
    }
}
